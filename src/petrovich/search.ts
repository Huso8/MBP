...
import type { Product, SortMode } from './types.js';

function isDnsResolveError(error: unknown): boolean {
 ...
 */
async function cardTextAroundLink(link: Locator): Promise<string> {
	...
	const fallback = await link.evaluate((el: Element) => {
		let n: Element | null = el;
		for (let i = 0; i < 14 && n; i++) {
			const raw = n.textContent || '';
			if (raw.includes('₽') && /\d/.test(raw)) return raw.replace(/\s+/g, ' ').trim();
			n = n.parentElement;
		}
		return '';
	});
...
function absProductUrl(href: string | null | undefined): string {
	if (!href || href === '#') return '#';
...
async function gotoWithFallbacks(page: Page, query: string): Promise<void> {
 ...
 */
export async function searchInPetrovich(query: string, sortMode: SortMode = 'popular'): Promise<Product[]> {
	...
	try {
		...
		const rows: { product: Product; priceNum: number | null }[] = [];
		...
		for (let i = 0; i < Math.min(linkCount, 80); i++) {
			...
			if (title.length < 4 || title.startsWith('Тип товара:')) continue;
			seen.add(href);
		...
		if (isDnsResolveError(error)) throw new Error('DNS_RESOLVE_FAILED');
