...
const searchCache = new TtlCache<Product[]>(120_000);
...
const botCart = new Map<number, { url: string; name: string; addedAt: number }[]>();
...
bot.command('start', (ctx) => {
	ctx.reply(
		'👷‍♂️ Бот для поиска в Петровиче (Москва)\n\n' +
...
bot.on('message:text', async (ctx) => {
	...
	try {
		...
		const resMap = new Map<string, { url: string; name: string }>();
		...
		console.error(error);
		const messageText =
			error instanceof Error && error.message === 'DNS_RESOLVE_FAILED'
